...
private:
	enum MONSTER_STATE { STATE_MOVE, STATE_JUMP, STATE_ATTACK, STATE_STAY, STATE_DEAD, STATE_END };
	enum STATE_MAXFRAME { MAX_MOVERUN = 6, MAX_JUMP = 6, MAX_ATTACK = 12, MAX_DEAD = 7 };
	enum JUMP_STATE { JUMP_BEFORE, JUMP_ING, JUMP_AFTER };
	enum ATTACKPATTERN { ATTACK_JUMP, ATTACK_NORMAL, ATTACK_END };
	...
	ATTACKPATTERN	m_eAttackPattern = { ATTACKPATTERN::ATTACK_JUMP };
	_bool			m_bJump = false;
	_bool			m_bJumpEnd = false;
	_float			m_fJumpFinished = {};
	CCollider*		m_pAttackCollider = { nullptr };
	_bool			m_isCloseAttack = { false };
	_bool			m_bOneMoreAttack = { false };
