...
CDust::CDust(LPDIRECT3DDEVICE9 pGraphicDev, wstring _strObjName)
	:CPSystem(pGraphicDev, _strObjName)
{
}

CDust::CDust(const CPSystem& Prototype)
	: CPSystem(Prototype)
{
}
...
void CDust::resetParticle(Attribute* attribute)
{
	...
	attribute->_LifeTime = 3.f;							// 라이프타임
}

EVENT CDust::Update(_float timeDelta)
{
...
CDust* CDust::Create(LPDIRECT3DDEVICE9 pGraphicDev, wstring _strObjName, int numparticles, _float3 MinBox, _float3 MaxBox)
{
	CDust* pInstance = new CDust(pGraphicDev, _strObjName);
