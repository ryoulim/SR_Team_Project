...
void CButton_Main::Initialize_ButtonRect()
{
	...
	m_tButton[GAMESTART].right = m_tButton[GAMESTART].left + m_vSize.x;
	m_tButton[GAMESTART].bottom = m_tButton[GAMESTART].top + m_vSize.y;

	m_tButton[OPTIONS].left = LONG(posx);
	m_tButton[OPTIONS].top = LONG(posy - (m_vSize.y * 0.5f) + 60.f);
	m_tButton[OPTIONS].right = m_tButton[OPTIONS].left + m_vSize.x;
	m_tButton[OPTIONS].bottom = m_tButton[OPTIONS].top + m_vSize.y;

	m_tButton[QUIT].left = LONG(posx);
	m_tButton[QUIT].top = LONG(posy - (m_vSize.y * 0.5f) + 120.f);
	m_tButton[QUIT].right = m_tButton[QUIT].left + m_vSize.x;
	m_tButton[QUIT].bottom = m_tButton[QUIT].top + m_vSize.y;
...
HRESULT CButton_Main::Pick_Button()
{
	...
	{
		RENDER_TEXT_BOC("New Game Start", 0.f, 0.f, 1.f);
	...
	if (PtInRect(&m_tButton[QUIT], ptMouse))
	{
		...
		{
			RENDER_TEXT_BOC("quit", 0.f, 0.f, 1.f);
		}
	}
