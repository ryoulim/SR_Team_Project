...
EVENT CTtakkeun_i::Update(_float fTimeDelta)
{
	...
	if (m_bDead)
	{
		...
		{
			CFXMgr::Get_Instance()->SpawnCustomExplosion(vPos, m_eLevelID, _float3{ 60.f, 100.f, 1.f }, TEXT("Effect_Explorer"), 24);
			m_fCallTimer = 0.f;
		}
		if (m_fTotalTime >= 3.f)
		{
			_float3 vImpactPos = *m_pTransformCom->Get_State(CTransform::STATE_POSITION);
			CFXMgr::Get_Instance()->SpawnCustomExplosion(vImpactPos, m_eLevelID, _float3{ 200.f, 250.f, 1.f }, TEXT("Effect_Explor"), 32);
			m_bDie = true;
		}
	}
...
void CTtakkeun_i::FireAttack(_float dt)
{
	...
	/* 2. 화염을 발사한다! */
	CFXMgr::Get_Instance()->FireAttack(vMyPos, m_eLevelID, m_iNum);
...
void CTtakkeun_i::JumpAttack(_float dt)
{
	...
	if (m_bIsFly && m_bIsLink)//연결 true
	{
		...
		else
		{
			//이펙트 생성 && 카메라 쉐이킹
			vPos.y = 30.f;
			CFXMgr::Get_Instance()->JumpAttack(vPos, m_eLevelID);
...
void CTtakkeun_i::On_Collision(_uint MyColliderID, _uint OtherColliderID)
{
	...
	else if (CI_WEAPON(OtherColliderID))
	{
		...
		// 이펙트 생성
		m_iHP += -10;
		CFXMgr::Get_Instance()->SpawnBlood(vImpactPos, m_eLevelID);
	}
