...
HRESULT CRaceBoss::Initialize(void* pArg)
{
	...
	m_pSoundCom->SetVolume("BombStart", 1.8f);//사용됨
	m_pSoundCom->SetVolume("Die", 0.3f);
	m_pSoundCom->SetVolume("Hit", 0.6f);
	m_pSoundCom->SetVolume("MombackReady", 1.0f);//사용됨
	m_pSoundCom->SetVolume("BombAttack", 1.0f);//사용됨
	m_pSoundCom->SetVolume("Lazer2", 0.4f);
	m_pSoundCom->SetVolume("Lazer3", 0.25f);
	m_pSoundCom->SetVolume("CrossAttack", 2.5f);//사용됨
	m_pSoundCom->SetVolume("Momback", 0.35f);
	m_pSoundCom->SetVolume("ShotBullet", 0.8f);//사용됨
