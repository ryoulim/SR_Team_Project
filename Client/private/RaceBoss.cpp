...
HRESULT CRaceBoss::Initialize(void* pArg)
{
	...
	m_pSoundCom->SetVolume("BombStart", 2.f);//사용됨
	m_pSoundCom->SetVolume("Die", 0.4f);
	m_pSoundCom->SetVolume("Hit", 0.7f);
	m_pSoundCom->SetVolume("MombackReady", 3.f);//사용됨
	m_pSoundCom->SetVolume("BombAttack", 0.8f);//사용됨
	m_pSoundCom->SetVolume("Lazer2", 0.6f);
	m_pSoundCom->SetVolume("Lazer3", 0.45f);
	m_pSoundCom->SetVolume("CrossAttack", 5.f);//사용됨
